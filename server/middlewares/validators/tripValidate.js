...
import {
  joiErrorsTemplate,
  cityErrorTemplate,
} from '../../services/templates/TripsErrorTemplate';
...
class TripValidations {
  ...
  static async createTripCityAndDateCheck(req, res, next) {
    ...
    const findDestination = await cityService.findByProp({
  ...
  static async checkOpenTripCityAndDATE(req, res, next) {
    let city;
    if (req.body.origin) {
      const findOrigin = await cityService.findByProp({
  ...
  static async validateApproveData(req, res, next) {
    const { error } = approveDataValidateSchema.validate({
