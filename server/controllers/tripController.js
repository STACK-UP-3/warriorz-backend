...
class TripController {
  ...
  static async approveRejectTripRequest(req, res) {
    const { status, requestId } = req.body;
    const updatedRequest = await reqService.updateAtt(
      { status },
      { id: requestId },
    );
    const message = `Trip request ${status} successfully`;
    util.setSuccess(200, message, updatedRequest);
    return util.send(res);
  }
}
