...
before(async () => {
    ...
    await photoService.createphoto({ ownerId: userWithPhoto.dataValues.id, type: 'user', url: 'https://res.cloudinary.com/dcna45id5/image/upload/v1586427877/samples/landscapes/nature-mountains.jpg' });
    token = encode({ ...profileTestData[0], id: user.dataValues.id });
    notVerifToken = encode({ ...profileTestData[1], id: notVerifieduser.dataValues.id });
    userwithPhotoToken = encode({ ...profileTestData[2], id: userWithPhoto.dataValues.id });
    withoutToken = encode({ ...profileTestData[3], id: userWithoutToken.dataValues });
    await userService.updateAtt({ token }, { id: user.dataValues.id })
    await userService.updateAtt({ token: notVerifToken }, { id: notVerifieduser.dataValues.id })
    await userService.updateAtt({ token: userwithPhotoToken }, { id: userWithPhoto.dataValues.id })
    await userService.updateAtt({ token: notFoundUserToken }, { id: notFoundUser.dataValues.id })
})
...
describe('=====test route /api/v1/profile - update profile info======', () => {
    ...
    it('Should return 403 status code when user is not verified', done => {
        chai.request(app)
            ...
            .end((err, res) => {
                ...
                expect(res.type).to.equal('application/json');
                expect(res.body).to.have.property('status');
                expect(res.body).to.have.property('message').to.equal('Please verify your account first!');
                done()
            })
    });
    ...
    it('Should return 400 status code when special character are used', done => {
        chai.request(app)
            ...
            .end((err, res) => {
                ...
                expect(res.body).to.have.property('status');
                expect(res.body).to.have.property('message');
                expect(res.body.message).to.have.property('error').to.equal('Incorrect use of special characters')
                done()
            })
    });
    ...
    it('Should return 401 when user is not logged in', done => {
        chai.request(app)
            ...
            .end((err, res) => {
                ...
                expect(res.type).to.equal('application/json');
                expect(res.body).to.have.property('status');
                expect(res.body).to.have.property('message').to.equal('Unauthorised Access: You have to login to Proceed')
                done()
            })
    });
    ...
    it('Should return 400 status code when empty data is given', done => {
        chai.request(app)
            ...
            .end((err, res) => {
                ...
                expect(res.type).to.equal('application/json');
                expect(res.body).to.have.property('status');
                expect(res.body).to.have.property('message').to.equal('Can not update empty data!')
                done()
            })
    });
    ...
    it('Should return 400 when photoUrl is not image url', done => {
        chai.request(app)
            ...
            .end((err, res) => {
                ...
                expect(res.type).to.equal('application/json');
                expect(res.body).to.have.property('status');
                expect(res.body).to.have.property('message').to.equal('The userPhoto is not image url with(jpeg,jpg,png)!')
                done()
            })
    });
    ...
    it('Should update profile when authenticated and data are valid', done => {
        chai.request(app)
            ...
            .end((err, res) => {
                ...
                expect(res.body).to.have.property('message').to.equal('Profile updated successfuly');
                done()
            })
    });
    ...
    it('Should update profile with photo url', done => {
        chai.request(app)
            ...
            .send({ photoUrl: 'https://res.cloudinary.com/dcna45id5/image/upload/v1586427877/samples/landscapes/nature-mountains.jpg' })
            .end((err, res) => {
                expect(res.body).to.have.status(200);
                expect(res.type).to.equal('application/json');
                expect(res.body).to.have.property('status');
